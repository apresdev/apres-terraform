# Getting Started with Apres - Part 2 - Apres Foundations

## Table of Contents

1. [Setup Customizations for AWS Control Tower (CfCT)](#1-setup-customizations-for-aws-control-tower-cfct)
...
Apres uses Terraform to configure and deploy services. Terraform
requires AWS credentials along with a state management mechanism.

Also, Service Control Policies are a powerful way to put guardrails around
your AWS organization.

To bootstrap both Terraform and Service Control Policies, Apres recommends
...
Steps to set up Customizations for AWS Control Tower (CfCT) follow.
...
The high-level steps are:
  ...
  1. Create and configure a new GitHub repository in your org based on an
    Apres template repository, link it to CfCT
  1. Use that repo to deploy Terraform state and AWS IAM artifacts that grant
    permissions to your repositories
  1. Create a second GitHub repository based on the Apres Foundations
     template  repository to deploy Terraform resources to your AWS accounts
  1. Begin deploying your services

## 1. Setup Customizations for AWS Control Tower (CfCT)
...
at [Step 1. Launch the stack](https://docs.aws.amazon.com/controltower/latest/userguide/step1.html), with two key modifications:
  ...
  1. Step 2 references regions. Use the region selector pick the home
     region you selected earlier.
  2. Under *Parameters* change *AWS CodePipeline Source* to *AWS CodeCommit*.

The rest of the options should be left as defaults.

## 2. Create a GitHub repository to manage core infrastructure

In this step you will be creating a repository which will contain Terraform
code to manage core infrastructure, including:
...
Let's review what you've setup so far:
  ...
  * set up a repository based on the Apres Foundations template
  * set up a repository for CfCT, and deployed to your AWS accounts:
    * A GitHub OIDC connector
