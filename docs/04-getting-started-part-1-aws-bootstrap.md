...
7. [Enable IAM access to billing](#7-enable-iam-access-to-billing)
...
Remember to add yourself to the distribution list, as you will need to
receive emails in the next step.
...
* Billing
* Integration with your organization's authentication (Google Workspace,
  Microsoft 365, etc.)
...
not be able to log in by guessing or obtaining your password.
...
It is critical that you do not use the Root user account for day-to-day
activities. For now, you will
...
2. [Enabling all features in your organization](https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_org_support-all-features.html)
...
By default, access to the billing information (including AWS Cost Explorer)
is not enabled for non-root users. Also, it is recommended that you **not**
log in as the root user for day-to-day administration. To that end, we need
to grant access to billing to the IAM-based administrators.

Follow these steps: [Granting access to your billing information and tools]
(https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/control-access-billing.html#grantaccess)

## 8. Enable Integration to your Identity Provider
...
1. In the AWS Console in the root account, navigate to the AWS Control Tower service.
...
Once Control Tower is set up, Apres recommends following AWS's recommended
