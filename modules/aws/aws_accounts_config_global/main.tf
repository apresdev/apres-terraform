...
data "aws_iam_policy_document" "cloudwatch_assume_role" {
  count = var.observe_account_id == "" ? 0 : 1
  statement {
    actions = ["sts:AssumeRole"]
    effect  = "Allow"
    principals {
      type        = "AWS"
      identifiers = ["arn:aws:iam::${var.observe_account_id}:root"]
    }
  }
}
...
data "aws_iam_policy_document" "lambda_assume_role" {
  ...
  statement {
    ...
    condition {
      ...
      values   = ["arn:aws:iam::${var.observe_account_id}:role/ApresGrafanaConfiguratorLambda"]
    }
  }
}
