...
resource "aws_iam_policy" "chatbot_guardrails" {
  ...
  name_prefix = "${var.environment}-${var.name}-ChatBot-Guardrails"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "awscc_chatbot_slack_channel_configuration" "default" {
  ...
  configuration_name = "${lower(module.apres_names.local_name)}-${var.slack_channel_id}"
...
resource "awscc_chatbot_microsoft_teams_channel_configuration" "default" {
  ...
  configuration_name = "${lower(module.apres_names.local_name)}-${var.msteams_channel_id}"
