...
This module supports associating Lambda@Edge functions with the CloudFront distribution via the `cloudfront_cache_lambda_config` variable. This variable takes a list of maps, each specifying:

- `event_type`: One of `viewer-request`, `origin-request`, `viewer-response`, `origin-response` (required)
- `lambda_arn`: ARN of the Lambda function (must be a valid Lambda function ARN, required)
- `include_body`: (optional) Boolean, whether to include the body, defaults to false

Example usage:

```hcl
cloudfront_cache_lambda_config = [
  {
...
This will create an `ordered_cache_behavior` with a `lambda_function_association` for each entry.
...
| <a name="input_cloudfront_cache_lambda_config"></a> [cloudfront\_cache\_lambda\_config](#input\_cloudfront\_cache\_lambda\_config) | List of Lambda@Edge associations for the CloudFront cache behavior. Each map must have:<br>    - event\_type: One of "viewer-request", "origin-request", "viewer-response", "origin-response"<br>    - lambda\_arn: ARN of the Lambda function (must be a valid Lambda function ARN)<br>    - include\_body: (optional) Boolean, whether to include the body, defaults to false<br>  Example:<br>    [<br>      {<br>        event\_type   = "viewer-request"<br>        lambda\_arn   = "arn:aws:lambda:us-east-1:123456789012:function:my-function:1"<br>        include\_body = false<br>      }<br>    ] | <pre>list(object({<br>    event_type   = string<br>    lambda_arn   = string<br>    include_body = optional(bool, false)<br>  }))</pre> | `[]` | no |
